...
def main():
    ...
    # quantize code
    if args.quantize:
        tuner = QuantTune(model,args.wgt_target,args.act_target,layerwise=args.layerwise,duration=args.duration,logger=logger,device=torch.device(0))
    else:
        tuner = None
    ...
    if args.resume:
        ...
        #     model.load_state_dict(checkpoint['state_dict'])
        #     logger.info("=> loaded checkpoint '{}' (epoch {}) with best acc@1: {}"
        #         .format(args.evaluate, checkpoint['epoch'],best_prec1))
        # else:
        #     logger.info("=> no checkpoint found at '{}'".format(args.resume))
        checkpoint = torch.load(args.resume) 
        model.load_state_dict(checkpoint['state_dict'])
        logger.info('==> load resume model')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, criterion, optimizer, epoch,logger,tuner)
        lr_scheduler.step()
        if args.quantize and tuner.quantize_cur:
            tuner.sort_and_tune(epoch)
        if args.quantize:
...
def train(train_loader, model, criterion, optimizer, epoch,logger,tuner=None):
    """
        Run one train epoch
    """
